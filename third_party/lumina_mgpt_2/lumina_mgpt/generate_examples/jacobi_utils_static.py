    ...
    class JacobiSampler(model_class, nn.Module):
        ...
        def _init_new_params(
            ...
            **kwargs,
            ...
            self.jacobi_image_block_mode = bool(jacobi_image_block_mode)
            if self.is_compile:
        ...
        def _sample(
            ...
            **model_kwargs,
            ...
            # Only the score of the last accepted token is consumed after the
            # prefill step.  Keeping a dense [prompt_len, vocab_size] one-hot
            # tensor is prohibitively expensive for multimodal prompts (for
            # example, ~9.8k x ~171k entries in ImageFusion-R1).
            temporary_collected_scores = torch.zeros(
                (batch_size, 1, self.config.vocab_size),
                dtype=torch.float32,
                device=input_ids.device,
            )
            temporary_collected_scores.scatter_(2, input_ids[:, -1:].unsqueeze(-1), 1.0)
            ...
            while self._has_unfinished_sequences(
                ...
                if self.jacobi_image_block_mode:
                    # ImageFusion-R1 emits three interleaved image blocks whose
                    # text prefixes have variable lengths.  Select the Jacobi
                    # window from the actual accepted prefix instead of one
                    # fixed prompt-relative interval.
                    accepted_ids = torch.cat(
                        [all_collected_input_ids, matched_next_tokens], dim=-1
                    )
                    ...
                    if image_processor is not None:
                        sequence = accepted_ids[0]
                        start_count = int(
                            (sequence == image_processor.image_start_token_id).sum().item()
                        )
                        end_count = int(
                            (sequence == image_processor.image_end_token_id).sum().item()
                        )
                        if start_count == end_count + 1:
                            start_index = int(
                                torch.where(
                                    sequence == image_processor.image_start_token_id
                                )[0][-1].item()
                            )
                            tokens_after_start = int(sequence.shape[0] - start_index - 1)
                            # The first two tokens encode the H/W grid.  Draft
                            # image tokens only after both have been accepted.
                            if tokens_after_start >= 2:
                                total_after_start = (
                                    2
                                    + image_processor.h_latent_dim
                                    * (image_processor.w_latent_dim + 1)
                                    + 1
                                )
                                remaining = total_after_start - tokens_after_start
                                output_token_num = max(
                                    1, min(max_num_new_tokens, remaining)
                                )
                elif (cur_len >= jacobi_loop_interval_lr[0]) and (cur_len < jacobi_loop_interval_lr[-1]):
