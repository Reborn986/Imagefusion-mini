...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def forward(
        ...
        ```"""
        # RL log-prob replay needs the transformer activations without eagerly
        # materializing [batch, sequence, vocab] logits.  Keeping this branch on
        # the root module is important for FSDP: root pre/post-forward hooks still
        # run, while the caller can project small token chunks through lm_head.
        if return_last_hidden_state:
            if labels is not None:
                raise ValueError("return_last_hidden_state does not accept labels")
            outputs = self.model(
                input_ids=input_ids,
