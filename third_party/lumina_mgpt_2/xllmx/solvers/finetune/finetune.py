...
class FinetuneSolverBase(ABC):
    ...
    def get_args_parser(cls):
        parser = argparse.ArgumentParser("xllmx Finetuning")
    ...
    def setup_fsdp_sync(
        ...
        if fsdp_cpu_offload:
            self.logger.info("FSDP CPU offload enabled via FSDP_CPU_OFFLOAD=1")
        fsdp_backward_prefetch = {
            "none": None,
            "post": BackwardPrefetch.BACKWARD_POST,
            "pre": BackwardPrefetch.BACKWARD_PRE,
        }[os.environ.get("FSDP_BACKWARD_PREFETCH", "pre").lower()]
        self.logger.info(f"FSDP backward_prefetch={fsdp_backward_prefetch}")
